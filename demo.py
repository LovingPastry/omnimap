...
def save_trajectory(omni, all_inputs, output):
    ...
    ttraj_full = [
        all_inputs[i]["pose"].cpu().numpy()[0] for i in range(len(all_inputs))
    ]
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=float,
        default=7.0,
        help="the max depth used for depth image",
    )
    parser.add_argument(
        "--cropborder", type=int, default=0, help="crop images to remove black border"
    )
    parser.add_argument("--start", type=int, default=0, help="start frame")
    parser.add_argument(
        "--length", type=int, default=100000, help="number of frames to process"
    )
    parser.add_argument(
        "--vis_gui",
        action="store_true",
    ...
    if args.dataset == "replica":
        ...
        args.cropborder = 10
    elif args.dataset == "rtabmap":
        rgbdir = f"{dataset_dir}/{args.scene}/imap/00/rgb"
        depthdir = f"{dataset_dir}/{args.scene}/imap/00/depth"
        pose = f"{dataset_dir}/{args.scene}/imap/00/traj_w_c.txt"
        args.calib = "calib/realsense.txt"
    else:
    ...
    for frame_data in all_inputs:
        t, image, depth, pose, intrinsics, pose_44, is_last = (
            frame_data["index"],
            frame_data["image"],
            frame_data["depth"],
            frame_data["pose"],
