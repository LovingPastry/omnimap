...
def timeit(
    func=None, *, sync_cuda=None, every=None, logger_name="omnimap.profile.timeit"
):
    """Timing decorator with optional CUDA synchronization and rolling average.

    Env knobs:
    - OMNIMAP_TIMEIT_SYNC_CUDA: 1/0 (default: 1)
    - OMNIMAP_TIMEIT_EVERY: positive int (default: 1)
    """
    ...
    if sync_cuda is None:
        sync_cuda = _env_flag("OMNIMAP_TIMEIT_SYNC_CUDA", True)
    if every is None:
        every = _env_int("OMNIMAP_TIMEIT_EVERY", 1)
    ...
    def _decorate(target):
        ...
        def wrapper(*args, **kwargs):
            ...
            if stats["count"] % max(1, int(every)) == 0:
                avg = stats["total"] / max(stats["count"], 1)
                logger = logging.getLogger(str(logger_name))
                message = (
                    f"[{target.__qualname__}] elapsed={elapsed:.6f}s "
                    f"avg={avg:.6f}s max={stats['max']:.6f}s n={stats['count']}"
                )
                if logger.handlers or logger.parent is not None:
...
class VisualModule:
    ...
    def erode_mask(self, mask, kernel_size=5):
        ...
        eroded_mask = F.conv2d(
            mask.unsqueeze(0).unsqueeze(0).float(), kernel, padding=kernel_size // 2
        )
...
class YOLOVisualModule(VisualModule):
    ...
    def detect(self, img, vis_gui=False):
        ...
        min_rects = (
            torch.unique(min_rects, dim=0).cpu().numpy()
        )  # min_rects是最终检测结果
