...
import cv2
from gaussian.utils.graphics_utils import (
...
class Camera(nn.Module):
    ...
    def depths_to_points(self, depth=None, world_frame=False):
        ...
        fx = W / (2 * math.tan(self.FoVx / 2.0))
        fy = H / (2 * math.tan(self.FoVy / 2.0))
        intrins = (
...
class HemisphereCamera(Camera):
    ...
    角度单位：弧度
    坐标系约定（Z-up）：
    theta: 绕 z 轴方位角，范围 [0, 2π)
    phi: 相对 XY 平面的仰角，范围 [0, π/2]（上半球 z >= 0）
    """
    ...
    def update_pose_from_spherical(self):
        # 约束角度范围：theta in [0, 2pi), phi in [0, pi/2]
        phi = torch.clamp(self.phi, 0.0, math.pi / 2.0)
        theta = torch.remainder(self.theta, 2.0 * math.pi)

        # 球坐标 -> 笛卡尔（Z-up）
        x = self.radius * torch.cos(phi) * torch.cos(theta)
        y = self.radius * torch.cos(phi) * torch.sin(theta)
        z = self.radius * torch.sin(phi)
        ...
        right_unnorm = torch.cross(world_up, forward, dim=0)
        if torch.linalg.norm(right_unnorm) < 1e-6:
            # near pole: fallback to an alternative up axis to avoid degeneracy
    ...
    def set_angles(self, theta=None, phi=None):
        with torch.no_grad():
            if theta is not None:
                wrapped_theta = float(theta) % (2.0 * math.pi)
                self.theta.copy_(torch.tensor([wrapped_theta], device=self.device))
            if phi is not None:
                clamped_phi = max(0.0, min(float(phi), math.pi / 2.0))
                self.phi.copy_(torch.tensor([clamped_phi], device=self.device))
        self.update_pose_from_spherical()

    def get_angles(self):
        """返回(theta, phi)，其中theta∈[0,2π), phi∈[0,π/2]。"""
    ...
    def from_camera(
        ...
        theta = torch.atan2(rel_pos_normalized[1], rel_pos_normalized[0])
        theta = torch.remainder(theta, 2.0 * math.pi)
        phi = torch.asin(torch.clamp(rel_pos_normalized[2], 0.0, 1.0))
