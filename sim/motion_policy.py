...
def resolve_step_scales(
    unified_scale: float,
    *,
    theta_scale: float | None = None,
    phi_scale: float | None = None,
) -> tuple[float, float]:
    """Resolve a shared Fisher step scale plus optional per-axis overrides."""
    if not np.isfinite(unified_scale):
        raise ValueError("fisher unified step scale must be finite")
    resolved_theta = float(unified_scale if theta_scale is None else theta_scale)
    resolved_phi = float(unified_scale if phi_scale is None else phi_scale)
    if not np.isfinite(resolved_theta) or not np.isfinite(resolved_phi):
        raise ValueError("resolved Fisher step scales must be finite")
    return resolved_theta, resolved_phi
...
def _position_to_spherical(
    ...
    if radius < 1e-12:
        raise ValueError("position is too close to scene_center; cannot recover spherical state")
    n_hat = offset / radius
...
class FisherMotionPolicy:
    ...
    def __init__(
        ...
        *,
        ...
        fallback_delta_theta: float = 0.03,
        fallback_delta_phi: float = 0.0,
        ...
        if radial_gain < 0 or not np.isfinite(radial_gain):
            raise ValueError(f"radial_gain must be non-negative and finite, got {radial_gain}")
        if grad_eps <= 0:
        ...
        if fallback_delta_theta < 0 or fallback_delta_phi < 0:
            raise ValueError("fallback deltas must be non-negative")
        ...
        self.fallback_delta_theta = float(fallback_delta_theta)
        self.fallback_delta_phi = float(fallback_delta_phi)
        self.grad_norm_epsilon = float(grad_norm_epsilon)
    ...
    def _initialize_reference_geometry(
        ...
        *,
        ...
        if self.reference_initialized:
            return
        self.reference_scene_center = np.asarray(scene_center_np, dtype=np.float64).copy()
        self.reference_radius = float(
            np.linalg.norm(np.asarray(current_position, dtype=np.float64) - self.reference_scene_center)
        )
    ...
    def _build_cartesian_result(
        ...
        *,
        ...
        current_radius, _, _ = _position_to_spherical(current_position, reference_scene_center)
        ...
        theta_rate_unclipped = self.step_gain_theta * grad_theta
        phi_rate_unclipped = self.step_gain_phi * grad_phi
        theta_rate = self._clip_signed(theta_rate_unclipped, self.max_delta_theta / self.dt)
        phi_rate = self._clip_signed(phi_rate_unclipped, self.max_delta_phi / self.dt)

        if fallback_used:
            theta_rate = self.fallback_delta_theta / self.dt
            phi_rate = self.fallback_delta_phi / self.dt

        vt_world = current_radius * (theta_rate * e_theta + phi_rate * e_phi)
        vn_scalar = -self.radial_gain * radial_error
        vn_world = vn_scalar * n_hat
        velocity_world = vt_world + vn_world
        ...
        next_c2w = _sim_look_at_c2w(eye=next_position, target=reference_scene_center)
        ...
        return MotionPolicyResult(
            ...
            clipped_theta=clipped_theta,
            clipped_phi=clipped_phi,
    ...
    def _to_hemisphere_camera(
        ...
        *,
        ...
        if isinstance(current_viewpoint, Camera):
            return HemisphereCamera.from_camera(current_viewpoint, scene_center), "camera"
    ...
    def next_pose(
        ...
        *,
        ...
        """Advance one step on the hemisphere using Fisher angle gradients.

        This is the core Phase-3/4 policy:
        1. recover the current view on the Fisher hemisphere
        2. query `dF/dtheta` and `dF/dphi`
        3. scale and clip the angular update
        4. apply a tiny fallback move if the field is locally flat
        5. convert the updated hemisphere state back into `next_c2w`
        """
        ...
        reference_scene_center = np.asarray(self.reference_scene_center, dtype=np.float64)
        ...
        # First map raw Fisher gradients into bounded control increments.
        delta_theta_unclipped = self.step_gain_theta * grad_theta
        delta_phi_unclipped = self.step_gain_phi * grad_phi
        delta_theta = self._clip_signed(delta_theta_unclipped, self.max_delta_theta)
        delta_phi = self._clip_signed(delta_phi_unclipped, self.max_delta_phi)
        clipped_theta = not math.isclose(
            delta_theta, delta_theta_unclipped, rel_tol=1e-9, abs_tol=1e-12
        )
        clipped_phi = not math.isclose(
            delta_phi, delta_phi_unclipped, rel_tol=1e-9, abs_tol=1e-12
        )
        fallback_used = False
        if grad_norm < self.grad_norm_epsilon:
            delta_theta = self.fallback_delta_theta
            delta_phi = self.fallback_delta_phi
            fallback_used = True
        ...
        if self.cartesian:
            ...
            if self.verbose:
                print(
                    ...
                    f"|v|={np.linalg.norm(result.velocity_world):.6f} "
        ...
        if self.verbose:
            print(
                ...
                f"grad=({result.grad_theta_raw:.6f}, {result.grad_phi_raw:.6f}) "
                f"delta=({result.delta_theta_applied:.6f}, {result.delta_phi_applied:.6f}) "
                f"score={result.fisher_score:.6f} "
                f"clip=({result.clipped_theta}, {result.clipped_phi}) "
                f"fallback={result.fallback_used}"
            )
        return result
