	...
	const int grad_power)
{
	...
	for (int i = 0; i < rounds; i++, toDo -= BLOCK_SIZE)
	{
		...
		for (int j = 0; j < min(BLOCK_SIZE, toDo); j++)
		{
			...
			float v_mean2d_x = skip ? 0.f : dL_ddelx * ddelx_dx;
			float v_mean2d_y = skip ? 0.f : dL_ddely * ddely_dy;
			float v_conic_x  = skip ? 0.f : -0.5f * gdx * d.x * dL_dG;
			float v_conic_y  = skip ? 0.f : -0.5f * gdx * d.y * dL_dG;
			float v_conic_w  = skip ? 0.f : -0.5f * gdy * d.y * dL_dG;
			float v_opacity  = skip ? 0.f : G * dL_dopa;

			// 若只想在 grad_power==1 时保持原样，可加条件
			if (grad_power != 1) {
				v_mean2d_x = powf(v_mean2d_x, grad_power);
				v_mean2d_y = powf(v_mean2d_y, grad_power);
				v_conic_x  = powf(v_conic_x,  grad_power);
				v_conic_y  = powf(v_conic_y,  grad_power);
				v_conic_w  = powf(v_conic_w,  grad_power);
				v_opacity  = powf(v_opacity,  grad_power);
			}

			dL_dmean2D_shared[tid].x = v_mean2d_x;
			dL_dmean2D_shared[tid].y = v_mean2d_y;
			dL_dconic2D_shared[tid].x = v_conic_x;
			dL_dconic2D_shared[tid].y = v_conic_y;
			dL_dconic2D_shared[tid].w = v_conic_w;
			dL_dopacity_shared[tid] = v_opacity;
	...
	const int grad_power)
{
    renderCUDA<NUM_CHANNELS, true> <<<grid, block>>> ( \
        ...
        dL_dts, dL_dray_planes, grad_power);
}